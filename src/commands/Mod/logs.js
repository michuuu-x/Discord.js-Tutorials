const {
    ...
    Client,
    Message
} = require('discord.js') // requring the discord.js package
...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!args.length) return message.reply("Invalid amount of arguments!")
        const opt = args[0].toLowerCase();
        if (!opt) return message.reply('Please provide a valid operation!')


        if (!options.includes(opt)) return message.reply('That is not a valid operation!')

        if (opt === 'set') {

            const channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[1]);
            if (!channel) return message.reply('Please provide the channel to set as!')
            ...
            db.findOne({
                ...
                if (!data) {
                    return message.reply(`The logs channel has not been set yet!!`)
                } else {
                    await db.findOneAndDelete({ guild: message.guild.id }, data)
                    data.save()
                    return message.reply(`The logs channel has been reseted and the logs command has been disabled!!`)
                }
